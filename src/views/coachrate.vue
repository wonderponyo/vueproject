<template>
    <div>
        <br><br> <br><br>
        <span id="msg">教练姓名：{{coachname}}</span><br> <br><br>
        <el-rate
                :disabled="flag"
                @change="rated"
                v-model="value2"
...
<script>
	...
	export default {
		...
		created() {
			...
			axios.get('http://localhost:8181/springboot/getcoach/2').then(function (resp) {
				console.log(resp.data);
				// _this.schoolid=resp.data.did;
				_this.coachname=resp.data.coach;
				if (resp.data.coachrated==="1") {
					_this.$notify.error({
						title: '您已完成评价,无法再次评价!',
						offset: 200
